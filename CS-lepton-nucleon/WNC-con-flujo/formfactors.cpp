...
void formfactors( double Qsq, double &F1p, double &F2p, double &F1n, double &F2n, double &GA, double &GP, double &F1s, double &F2s, double &GAs)
{
  ...
  double DipV, DipA, MA, MA2, MV, MV2;
  MA = 1.032;
  MA2 = MA*MA; //
  MV = 0.843;
  MV2 = MV*MV; 
...
//     iff = 0; // Dipole
  ...
  double GEs = rhos*tau*DipV;
  double GMs = mus*DipV;
  GAs = gas*DipA;
  
  F1s = (GEs+tau*GMs)/(1.+tau);
  F2s = (GMs-GEs)/(1.+tau);
  // // // // // // // // // // // //     
