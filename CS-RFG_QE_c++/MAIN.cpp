...
int main()
{
  ...
  out.open("Ei"+to_string(iEi)+"_th"+to_string(itheta)+".out"); //name of the output file
  out.precision(4); //number of digits in the printed numbers 
  // // // // // // // // // //
  
  for(double w=1; w<=Ei; w+=1)
  {
    ...
    // // final lepton // //
    double Ef = Ei-w;       //if(Ef >= Ei || Ef < 0){continue;}
    ...
    if(d3sig>0)
    {
      ...
      d3sig = d3sig*pow(hbc,2);// hbc^2 to get the cross section in fm^2/(MeV srad)
      
//       double d2sig = d3sig * 2.*Pi; 
      
      out << w << " " << kf << " " << d3sig << endl; //d3s/(dkf dOmegaf) ; units~(fm^2/MeV srad)
    }
