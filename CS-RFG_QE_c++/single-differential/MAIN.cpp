...
int main()
{
  ...
  for(double thetaf_deg=0; thetaf_deg<=180; thetaf_deg+=2)
  {
...
//       cout << "w= " << w << endl;
      ...
      d2sig = 2*Pi*d3sig*pow(hbc,2);// hbc^2 to get the cross section in fm^2/(MeV srad)
    ...
    out << thetaf_deg << " " << dsig << endl; //d3s/(dkf dOmegaf) ; units~(fm^2/MeV srad)
    cout << thetaf_deg << " " << dsig << endl;
